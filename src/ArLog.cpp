...
ArGlobalRetFunctor<bool>
    ArLog::ourAramConfigProcessFileCB(&ArLog::aramProcessFile);
...
#ifndef ARINTERFACE
  // check this down here instead of up in the if ourFP so that the log filled
  // shows up after the printf
...
AREXPORT void ArLog::logErrorFromOS(LogLevel level, const char *str, ...) {
  ...
  snprintf(bufWithError, sizeof(bufWithError) - 1,
           "%s | ErrorFromOSNum: %d ErrorFromOSString: %s", buf, err,
           errorString);
...
/**
   ...
   @param str printf() like formating string
*/
AREXPORT void ArLog::logErrorFromOSPlainNoLock(LogLevel level,
                                               const char *str) {
...
/**
   ...
   @param logTime if this is true then the time a message is given will be
   logged
   @param alsoPrint if this is true then in addition to whatever other logging
   (to a file for instance) the results will also be printed
   @param printThisCall if this is true the new settings will be printed
...
AREXPORT bool ArLog::init(LogType type, LogLevel level, const char *fileName,
  ...
  if (type == StdOut)
    ...
    if (fileName != NULL) {
      if (strcmp(ourFileName.c_str(), fileName) == 0) {
        ArLog::logNoLock(ArLog::Terse,
                         "ArLog::init: Continuing to log to the same file.");
      } else {
        close();
        if ((ourFP = ArUtil::fopen(fileName, "w")) == NULL) {
          ArLog::logNoLock(ArLog::Terse,
                           "ArLog::init: Could not open file %s for logging.",
                           fileName);
          ourMutex.unlock();
          return (false);
        }
    ...
    if (fileName) {        // need to translate fileName to integer index
    }
  } else if (type == None) {
  }
  ourType = type;
...
AREXPORT void ArLog::addToConfig(ArConfig *config) {
  ...
  config->addParam(
      ArConfigArg("LogType", (int *)&ourConfigLogType,
                  "The type of log we'll be using, 0 for StdOut, 1 for StdErr, "
                  "2 for File (and give it a file name), 3 for colbert (don't "
                  "use that), and 4 for None",
                  ArLog::StdOut, ArLog::None),
      section.c_str(), ArPriority::TRIVIAL);
  config->addParam(ArConfigArg("LogLevel", (int *)&ourConfigLogLevel,
                               "The level of logging to do, 0 for Terse, 1 for "
                               "Normal, and 2 for Verbose",
                               ArLog::Terse, ArLog::Verbose),
                   section.c_str(), ArPriority::TRIVIAL);
  config->addParam(ArConfigArg("LogFileName", ourConfigFileName,
                               "File to log to", sizeof(ourConfigFileName)),
                   section.c_str(), ArPriority::TRIVIAL);
  config->addParam(
      ArConfigArg(
          "LogTime", &ourConfigLogTime,
          "True to prefix log messages with time and date, false not to"),
      section.c_str(), ArPriority::TRIVIAL);
  config->addParam(ArConfigArg("LogAlsoPrint", &ourConfigAlsoPrint,
...
AREXPORT bool ArLog::processFile(void) {
  if (ourConfigLogType != ourType || ourConfigLogLevel != ourLevel ||
      ...
      ourConfigLogTime != ourLoggingTime ||
      ourConfigAlsoPrint != ourAlsoPrint) {
...
AREXPORT void ArLog::aramInit(const char *prefix, ArLog::LogLevel defaultLevel,
  ...
  Aria::getConfig()->addParam(
      ArConfigArg("Level", ourAramConfigLogLevel,
                  "The level of logging type of log we'll be using",
                  sizeof(ourAramConfigLogLevel)),
      section.c_str(), ArPriority::TRIVIAL, "Choices:Terse;;Normal;;Verbose");
  Aria::getConfig()->addParam(
      ArConfigArg("LogFileSize", &ourAramConfigLogSize,
                  "The maximum size of the log files (6 files are rotated "
                  "through), 0 means no maximum",
                  0, 20000),
      section.c_str(), ArPriority::TRIVIAL);
  ...
  ourAramLogSize =
      ArMath::roundInt(ourAramConfigLogSize * 1000000); // even megabytes
}
...
AREXPORT void ArLog::clearFunctor() { ourFunctor = NULL; }
...
AREXPORT void ArLog::log_v(LogLevel level, const char *prefix, const char *str,
                           va_list ptr) {
  char buf[1024];
